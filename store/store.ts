...
export type store = {
  ...
  setAuthStatus: (
    msg: string | undefined,
    _statusType: "success" | "info" | "error" | "warning" | undefined
  ) => void;
...
const useStore = create<store>((set) => ({
  ...
  setUserLoginInfo: (_uname, _password) =>
    set((state) => ({
      ...state,
      userLoginInfo: {
        username: _uname !== undefined ? _uname : state.userLoginInfo.username,
        password:
          _password !== undefined ? _password : state.userLoginInfo.password,
      },
    })),
  ...
  toggleUserCheckingOverlay: () =>
    set((state) => ({ userCheckingOverlay: !state.userCheckingOverlay })),

  sessionFilter: "today",
  setSessionFilter: (session_name) =>

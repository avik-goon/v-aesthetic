...
export async function isUserLoggedIn() {
  try {
    ...
    if (_.has(currentAuthenticatedUser, "signInUserSession")) return true;
  } catch (error: any) {
    return false;
  }
}
...
export class Admin {
  ...
  async handleLogin() {
    try {
      ...
      if (_.has(loginStatus, "signInUserSession")) return "SUCCESS";
    } catch (error: any) {
      return {
        status: "FAILED",
        msg: error.message,
  ...
  async changePassword(password: any, setIsLoginBtnPressed: any) {
    try {
      const user = await Auth.currentAuthenticatedUser();
      //@ts-ignore
      return await Auth.changePassword(user, this.password, password);
    } catch (e) {
